...
import static cn.howardliu.monitor.cynomys.net.struct.MessageType.SQL_INFO_REQ;
...
public class SqlInfo2KafkaHandler extends AbstractInfo2KafkaHandler {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Message message) throws Exception {
        ...
        if (header.getType() == SQL_INFO_REQ.value()) {
            if (logger.isDebugEnabled()) {
                logger.debug("{}-{}-{} send sql info",
                        header.getSysCode(), header.getSysName(), header.getTag());
            }
            send(ctx, message, SYSTEM_SETTING.getKafkaTopicSql(), MessageType.SQL_INFO_RESP);
        } else {
